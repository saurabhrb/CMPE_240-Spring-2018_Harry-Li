...
class TFT_22_ILI9225 {
	...
	public:
		...
		void setSPIfrequency(uint8_t freq);
		void rotate_point(int *x2,int *y2,int x1, int y1,int angle);
		void draw_polygon_fill(uint16_t px[], uint16_t py[],  uint16_t px_old[], uint16_t py_old[], uint8_t n, uint16_t color);

		void poly_fill(const VECTOR<Point3D> &poly, uint32_t color);
		void poly_fill3D(const VECTOR<Point3D> &poly, uint32_t color);

		bool max(int x, int y);
		bool min(int x, int y);
		int Is_Left(const Point3D &p0,const Point3D &p1,const Point3D &point);
		bool Is_Inside(const Point3D &point,const VECTOR<Point3D> &points_list);
		float dist2D(Point3D& P1,Point3D& P2);
		float dist3D(Point3D& P1,Point3D& P2);
		void poly_fill_diffuse(const VECTOR<Point3D> &V, uint32_t color,Point3D& Pz_min, Point3D& Pz_max);
